...
[MODIFIED] Added logging for misclassified URLs during evaluation.
"""
...
def build_features() -> tuple[sp.csr_matrix, np.ndarray, np.ndarray, HashingVectorizer]:
    ...
    log_memory(f"Final corpus created: {X.shape}, with {len(urls)} URLs")

    FEATURES_FILE.parent.mkdir(parents=True, exist_ok=True)
    sp.save_npz(FEATURES_FILE, X)
    np.save(LABELS_FILE, y)
    np.save(URLS_FILE, urls)
    log_memory("Feature, label, and URL caches written to disk.")
    return X, y, urls, hv
...
def load_or_build_features(
    ...
    if (
        ...
        urls = np.load(URLS_FILE)
        log_memory(f"Loaded X{X.shape}, y{y.shape}, and {len(urls)} URLs from cache.")
        return X, y, urls, hv

    log_memory(
        "Cache incomplete or --rebuild flag used. Building features from scratch..."
    )
...
def main(rebuild: bool) -> None:
    ...
    X_tr, X_te, y_tr, y_te, urls_te = train_test_split(
        X,
        y,
        urls,
        test_size=TEST_SIZE,
    ...
    misclassified_indices = np.where(y_te != preds)[0]
    if len(misclassified_indices) > 0:
        error_df = pd.DataFrame(
            {
                "url": urls_te[misclassified_indices],
                "true_label": y_te[misclassified_indices],
                "predicted_label": preds[misclassified_indices],
            }
        )
        error_df["true_label"] = error_df["true_label"].map(ID_TO_LABEL)
        error_df["predicted_label"] = error_df["predicted_label"].map(ID_TO_LABEL)

        error_log_path = REPORTS_DIR / "misclassified_urls.csv"
        error_df.to_csv(error_log_path, index=False)
        log_memory(f"Saved {len(error_df)} misclassified URLs to {error_log_path}")
    else:
        log_memory("Congratulations! No misclassified URLs found in the test set.")
